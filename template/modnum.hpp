...
template <int MOD> struct ModNum {
  ...
  ModNum &operator/=(const ModNum &rhs) { return *this *= mod_inv(rhs); }
  ModNum operator+() const { return *this; }
  ModNum operator-() const { return ModNum(v == 0 ? 0 : MOD - v); }
  ModNum inv() const {
    ModNum res;
    res.v = mod_inv_int<MOD>(v);
    return res;
  }
  friend ModNum inv(const ModNum &lhs) { return lhs.inv(); }
  ...
  friend std::ostream &operator<<(std::ostream &os, const ModNum &x) { return os << x.v; }
  bool operator<(const ModNum &lhs) const { return v < lhs.v; }
};
